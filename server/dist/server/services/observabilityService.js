...
class ObservabilityService {
    ...
    async recordMetric(name, value, unit, context = {}) {
        const metricEntry = {
            name,
            value,
            unit,
    ...
    async logError(error, context = { service: 'unknown', operation: 'unknown', severity: 'medium' }) {
        const errorEntry = {
    ...
    async getSecurityAnalytics(timeRange = '24h') {
        ...
        // Count failed logins
        const failedLogins = recentLogs.filter(l => l.message.includes('login') && l.level === 'WARN').length;
     ...
     */
    static async getAnalyticsDashboard(tenantId, timeRange = '24h') {
        try {
     ...
     */
    static async calculateRealAssessmentMetrics(tenantId, startTime, endTime) {
        try {
            ...
            const assessmentStats = await db
                ...
                .from(assessments)
                .where(and(eq(assessments.tenantId, tenantId), gte(assessments.createdAt, startTime), lte(assessments.createdAt, endTime)));
            ...
            const averageCompletionHours = completedWithTimes.length > 0
                ? Math.round(completedWithTimes.reduce((sum, a) => {
            ...
            const previousStats = await db
                ...
                .from(assessments)
                .where(and(eq(assessments.tenantId, tenantId), gte(assessments.createdAt, previousPeriodStart), lt(assessments.createdAt, startTime)));
     ...
     */
    static async calculateRealPerformanceMetrics(tenantId, startTime, endTime) {
        try {
            ...
            const performanceLogs = await db
                ...
                .from(systemLogs)
                .where(and(eq(systemLogs.tenantId, tenantId), eq(systemLogs.level, 'info'), gte(systemLogs.timestamp, startTime), lte(systemLogs.timestamp, endTime), like(systemLogs.message, '%response_time%')));
            ...
            const errorLogs = await db
                ...
                .from(systemLogs)
                .where(and(eq(systemLogs.tenantId, tenantId), eq(systemLogs.level, 'error'), gte(systemLogs.timestamp, startTime), lte(systemLogs.timestamp, endTime)));
     ...
     */
    static async calculateRealUserActivity(tenantId, startTime, endTime) {
        try {
            ...
            const userLogs = await db
                ...
                .from(systemLogs)
                .where(and(eq(systemLogs.tenantId, tenantId), gte(systemLogs.timestamp, startTime), lte(systemLogs.timestamp, endTime), isNotNull(systemLogs.userId)));
     ...
     */
    static async getRealSystemHealthMetrics() {
        try {
            ...
            const recentErrors = await db
                .select({ id: systemLogs.id })
                .from(systemLogs)
                .where(and(eq(systemLogs.level, 'error'), gte(systemLogs.timestamp, oneHourAgo)));
     ...
     */
    static async calculateRealComplianceMetrics(tenantId, startTime, endTime) {
        try {
            ...
            const complianceData = await db
                ...
                .where(and(eq(assessments.tenantId, tenantId), gte(answers.createdAt, startTime), lte(answers.createdAt, endTime)));
            ...
            const riskDistribution = complianceData.reduce((acc, answer) => {
                ...
                else if (complianceLevel === 'COMPLIANT')
                    acc.low = (acc.low || 0) + 1;
                return acc;
            }, {});
            // Calculate trend
            ...
            const previousData = await db
                ...
                .from(answers)
                .innerJoin(assessments, eq(answers.assessmentId, assessments.id))
                .where(and(eq(assessments.tenantId, tenantId), gte(answers.createdAt, previousPeriodStart), lt(answers.createdAt, startTime)));
     ...
     */
    static getFallbackAssessmentMetrics() {
        return {
            totalAssessments: 0,
    ...
    static getFallbackComplianceMetrics() {
        return {
     ...
     */
    static getStartTimeFromRange(timeRange, endTime) {
        const durationMs = {
            '1h': 60 * 60 * 1000,
            '6h': 6 * 60 * 60 * 1000,
            '24h': 24 * 60 * 60 * 1000,
     ...
     */
    static async recordMetric(metricName, value, unit, context) {
        try {
     ...
     */
    static async aggregateMetricRealTime(metricName, value, unit, context, timestamp) {
        try {
     ...
     */
    static async getHistoricalTrends(tenantId, metricName, timeRange = '7d') {
        try {
            const endTime = new Date();
            const startTime = this.getStartTimeFromRange(timeRange, endTime);
            ...
            const historicalData = await db
                ...
                .from(systemLogs)
                .where(and(eq(systemLogs.tenantId, tenantId), eq(systemLogs.level, 'info'), like(systemLogs.message, `metric:${metricName}%`), gte(systemLogs.timestamp, startTime), lte(systemLogs.timestamp, endTime)))
     ...
     */
    static async getPerformanceBaseline(tenantId) {
        try {
            ...
            const performanceData = await db
                ...
                .from(systemLogs)
                .where(and(eq(systemLogs.tenantId, tenantId), eq(systemLogs.level, 'info'), like(systemLogs.message, 'metric:response_time%'), gte(systemLogs.timestamp, thirtyDaysAgo)));
            // Calculate baselines
            ...
            const errorData = await db
                .select({ id: systemLogs.id })
                .from(systemLogs)
                .where(and(eq(systemLogs.tenantId, tenantId), eq(systemLogs.level, 'error'), gte(systemLogs.timestamp, thirtyDaysAgo)));
