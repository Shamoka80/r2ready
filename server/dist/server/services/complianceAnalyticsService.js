import { db } from '../db';
import { assessments, users, auditLog, evidenceFiles, systemLogs } from '@shared/schema';
...
export class ComplianceAnalyticsService {
     ...
     */
    async getComplianceKPIs(tenantId, timeRange = '30d') {
        try {
            ...
            const [templateUsage, templateCustomization, trainingCompletion, auditResults, incidentMetrics, userFeedback, complianceMetrics] = await Promise.all([
    ...
    async getDashboardMetrics(tenantId) {
        try {
            ...
            const [activeUsers] = await db
                .select({ count: count() })
                .from(systemLogs)
                .where(and(eq(systemLogs.tenantId, tenantId), gte(systemLogs.timestamp, oneHourAgo), isNotNull(systemLogs.userId)));
            const [documentsCreated] = await db
                .select({ count: count() })
                .from(evidenceFiles)
                .where(and(eq(evidenceFiles.tenantId, tenantId), gte(evidenceFiles.createdAt, oneDayAgo)));
            ...
            const [completedAssessments] = await db
                ...
                count: count(),
                criticalIssues: sql `COUNT(CASE WHEN overall_score < 60 THEN 1 END)`,
                recentFailures: sql `COUNT(CASE WHEN overall_score < 70 AND completed_at >= ${oneDayAgo} THEN 1 END)`
            })
                .from(assessments)
            ...
            const [performanceData] = await db
                ...
                count: count(),
                recentCompletions: sql `COUNT(CASE WHEN completed_at >= ${oneDayAgo} THEN 1 END)`
            })
                .from(assessments)
                .where(and(eq(assessments.tenantId, tenantId), eq(assessments.status, 'COMPLETED'), isNotNull(assessments.completedAt)));
            ...
            if (overallScore < 60)
                ...
                complianceAlerts += 1;
            if (recentFailures > 0)
                complianceAlerts += recentFailures;
            ...
            if (overallScore < 70)
                systemHealth -= 20;
            if (Number(assessmentsInProgress?.count || 0) > 10)
                systemHealth -= 10;
            if (complianceAlerts > 3)
                systemHealth -= 15;
    ...
    async calculateTemplateUsageRate(tenantId, startTime, endTime) {
        // Calculate how frequently templates are used vs. created from scratch
        const [templateUsage] = await db
            .select({
            templateUsed: sql `COUNT(CASE WHEN template_id IS NOT NULL THEN 1 END)`,
            totalDocuments: sql `COUNT(*)`
        })
            .from(evidenceFiles)
            .where(and(eq(evidenceFiles.tenantId, tenantId), gte(evidenceFiles.createdAt, startTime), lte(evidenceFiles.createdAt, endTime)));
    ...
    async calculateTemplateCustomizationFrequency(tenantId, startTime, endTime) {
        // Track modifications made to templates
        const [customizations] = await db
            .select({ count: count() })
            .from(auditLog)
            .where(and(eq(auditLog.tenantId, tenantId), eq(auditLog.action, 'TEMPLATE_MODIFIED'), gte(auditLog.timestamp, startTime), lte(auditLog.timestamp, endTime)));
        return { frequency: Number(customizations?.count || 0) };
    }
    async calculateTrainingCompletionRate(tenantId, startTime, endTime) {
    ...
    async calculateAuditPassRate(tenantId, startTime, endTime) {
        // Calculate audit success rates
        const [auditResults] = await db
            .select({
            passed: sql `COUNT(CASE WHEN overall_score >= 80 THEN 1 END)`,
            total: sql `COUNT(*)`
        })
            .from(assessments)
            .where(and(eq(assessments.tenantId, tenantId), eq(assessments.status, 'COMPLETED'), gte(assessments.completedAt, startTime), lte(assessments.completedAt, endTime)));
        const passRate = auditResults.total > 0 ?
            (auditResults.passed / auditResults.total) * 100 : 0;
        return { passRate: Math.round(passRate) };
    }
    async calculateIncidentResolutionTime(tenantId, startTime, endTime) {
        ...
        const [incidentData] = await db
            ...
            .from(auditLog)
            .where(and(eq(auditLog.tenantId, tenantId), eq(auditLog.action, 'INCIDENT_RESOLVED'), gte(auditLog.timestamp, startTime), lte(auditLog.timestamp, endTime)));
        return { avgTime: Math.round(Number(incidentData?.avgTime || 24)) };
    }
    async calculateUserSatisfactionScore(tenantId, startTime, endTime) {
        ...
        const [feedbackData] = await db
            ...
            .from(auditLog)
            .where(and(eq(auditLog.tenantId, tenantId), eq(auditLog.action, 'USER_FEEDBACK'), gte(auditLog.timestamp, startTime), lte(auditLog.timestamp, endTime)));
        return { score: Math.round(Number(feedbackData?.avgRating || 4.0) * 20) };
    }
    async calculateComplianceScore(tenantId, startTime, endTime) {
        ...
        const [complianceData] = await db
            ...
            .from(assessments)
            .where(and(eq(assessments.tenantId, tenantId), eq(assessments.status, 'COMPLETED'), isNotNull(assessments.overallScore), gte(assessments.completedAt, startTime), lte(assessments.completedAt, endTime)));
        return { score: Math.round(Number(complianceData?.avgScore || 0)) };
    }
    determineRiskLevel(complianceScore, auditPassRate, incidentTime) {
        const riskFactors = [];
        if (complianceScore < 60)
            riskFactors.push('low_compliance');
        if (auditPassRate < 70)
            riskFactors.push('audit_failures');
        if (incidentTime > 48)
            riskFactors.push('slow_resolution');
        if (riskFactors.length >= 3 || complianceScore < 40)
            return 'critical';
        if (riskFactors.length >= 2 || complianceScore < 60)
            return 'high';
        if (riskFactors.length >= 1 || complianceScore < 80)
            return 'medium';
        return 'low';
    }
    async detectAnomalies(tenantId) {
        ...
        const [documentActivity] = await db
            ...
            recentCount: sql `COUNT(CASE WHEN created_at >= ${oneDayAgo} THEN 1 END)`
        })
            .from(sql `(
        ...
        const [assessmentActivity] = await db
            .select({
            recentAssessments: sql `COUNT(CASE WHEN created_at >= ${oneDayAgo} THEN 1 END)`,
        ...
        const [scorePatterns] = await db
            .select({
            avgScore: avg(assessments.overallScore),
            recentScores: sql `COUNT(CASE WHEN overall_score < 60 AND completed_at >= ${oneWeekAgo} THEN 1 END)`
        })
            .from(assessments)
            .where(and(eq(assessments.tenantId, tenantId), eq(assessments.status, 'COMPLETED'), isNotNull(assessments.overallScore)));
    ...
    async assessComplianceRisk(tenantId) {
        const now = new Date();
        const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
        const [riskData] = await db
            .select({
            avgScore: avg(assessments.overallScore),
            criticalGaps: sql `COUNT(CASE WHEN overall_score < 60 THEN 1 END)`
        })
            .from(assessments)
            .where(and(eq(assessments.tenantId, tenantId), eq(assessments.status, 'COMPLETED'), gte(assessments.completedAt, thirtyDaysAgo)));
    ...
    async forecastResourceNeeds(tenantId) {
    ...
    async analyzeTrends(tenantId) {
        ...
        const [currentPeriodDocs] = await db
            .select({ count: count() })
            .from(evidenceFiles)
            .where(and(eq(evidenceFiles.tenantId, tenantId), gte(evidenceFiles.createdAt, thirtyDaysAgo)));
        const [previousPeriodDocs] = await db
            .select({ count: count() })
            .from(evidenceFiles)
            .where(and(eq(evidenceFiles.tenantId, tenantId), gte(evidenceFiles.createdAt, sixtyDaysAgo), lte(evidenceFiles.createdAt, thirtyDaysAgo)));
        ...
        const [previousScores] = await db
            .select({ avgScore: avg(assessments.overallScore) })
            .from(assessments)
            .where(and(eq(assessments.tenantId, tenantId), eq(assessments.status, 'COMPLETED'), gte(assessments.completedAt, sixtyDaysAgo), lte(assessments.completedAt, thirtyDaysAgo)));
            ...
            documentTrends: [
                {
                    ...
                    trend: docChangePercentage > 10 ? 'increasing' :
                        docChangePercentage < -10 ? 'decreasing' : 'stable',
                ...
                },
                {
                    ...
                    trend: scoreChangePercentage > 5 ? 'increasing' :
                        scoreChangePercentage < -5 ? 'decreasing' : 'stable',
            ...
            complianceTrends: [
                {
                    ...
                    direction: currentAvgScore > previousAvgScore ? 'improving' :
                        currentAvgScore < previousAvgScore ? 'declining' : 'stable',
                ...
                },
                {
                    ...
                    direction: docChangePercentage > 0 ? 'improving' :
                        docChangePercentage < 0 ? 'declining' : 'stable',
     ...
     */
    async getHistoricalTrends(tenantId) {
        const now = new Date();
        const trends = [];
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            AND created_at <= ${endDate}
                ...
                .from(assessments)
                .where(and(eq(assessments.tenantId, tenantId), eq(assessments.status, 'COMPLETED'), gte(assessments.completedAt, startDate), lte(assessments.completedAt, endDate)));
            trends.push({
