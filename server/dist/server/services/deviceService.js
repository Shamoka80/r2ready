...
export class DeviceService {
     ...
     */
    async logSecurityEvent(eventType, userId, deviceId, sessionId, metadata, ipAddress, userAgent, severity = 'info') {
        try {
            const auditEntry = {
                eventType,
                severity,
     ...
     */
    async registerDevice(userId, userAgent, ipAddress, deviceName, sessionId, additionalData) {
        const deviceFingerprint = this.generateDeviceFingerprint(userAgent, ipAddress, additionalData);
        // Check if device already exists
        const existingDevices = await db
            .select()
            .from(userDevices)
            .where(and(eq(userDevices.userId, userId), eq(userDevices.deviceFingerprint, deviceFingerprint), eq(userDevices.isRevoked, false)))
            .limit(1);
        if (existingDevices.length > 0) {
            ...
            const updatedDevice = await db
                ...
                .returning();
            await this.logSecurityEvent('device_login', userId, device.id, sessionId, {
                loginCount: device.loginCount + 1,
        ...
        await this.logSecurityEvent('device_registered', userId, device.id, sessionId, {
     ...
     * Trust a device
     */
    async trustDevice(deviceId, userId, sessionId, ipAddress, userAgent) {
        try {
            const result = await db
                ...
                .where(and(eq(userDevices.id, deviceId), eq(userDevices.userId, userId), eq(userDevices.isRevoked, false)))
                .returning();
            if (result.length === 0) {
                return { success: false, message: 'Device not found or already revoked' };
            }
            await this.logSecurityEvent('device_trusted', userId, deviceId, sessionId, {}, ipAddress, userAgent);
     ...
     * Revoke a device
     */
    async revokeDevice(deviceId, userId, reason, revokedBy, sessionId, ipAddress, userAgent) {
        try {
            const result = await db
                .update(userDevices)
                .set({
            ...
            await this.logSecurityEvent('device_revoked', userId, deviceId, sessionId, { reason, revokedBy }, ipAddress, userAgent, 'warning');
     ...
     * Update device name
     */
    async updateDeviceName(deviceId, userId, newName, sessionId, ipAddress, userAgent) {
        try {
            const result = await db
                .update(userDevices)
                .set({
            ...
            await this.logSecurityEvent('device_renamed', userId, deviceId, sessionId, { oldName: result[0].deviceName, newName }, ipAddress, userAgent);
     ...
     */
    async isDeviceTrusted(deviceId, userId) {
     ...
     */
    async getDeviceByFingerprint(userId, userAgent, ipAddress, additionalData) {
        const fingerprint = this.generateDeviceFingerprint(userAgent, ipAddress, additionalData);
        const devices = await db
            .select()
            .from(userDevices)
            .where(and(eq(userDevices.userId, userId), eq(userDevices.deviceFingerprint, fingerprint), eq(userDevices.isRevoked, false)))
            .limit(1);
     ...
     */
    async getSuspiciousDevices(userId) {
        // Get user's known IP ranges
        const knownDevices = await db
            .select({ ipAddress: userDevices.ipAddress })
            .from(userDevices)
            .where(and(eq(userDevices.userId, userId), eq(userDevices.isTrusted, true)));
        const knownIPClasses = new Set(knownDevices.map(device => this.getIPClass(device.ipAddress)));
        // Get recent devices
     ...
     */
    async cleanupOldDevices(olderThanDays = 90) {
        try {
            const cutoffDate = new Date();
            cutoffDate.setDate(cutoffDate.getDate() - olderThanDays);
            const result = await db
                .delete(userDevices)
                .where(and(eq(userDevices.isRevoked, true), lt(userDevices.revokedAt, cutoffDate)))
                .returning();
