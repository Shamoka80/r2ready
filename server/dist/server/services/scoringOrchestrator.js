...
export class ScoringOrchestrator {
     ...
     */
    static async calculateEnhancedScore(assessmentId, legacyCalculateFunction, intakeScope) {
        // Check feature flags
        ...
        if (useConfigWeights) {
            ...
            try {
                ...
                baseResult = {
                    ...
                    categoryScores: configResult.categoryScores.map((cat) => ({
     ...
     */
    static determineCategoryKey(question) {
     ...
     */
    static async adjustLegacyScoringForNAExclusion(assessmentId, legacyResult) {
        ...
        const categorizedAnswers = new Map();
        ...
        const adjustedCategoryScores = legacyResult.categoryScores.map((cat) => {
        ...
        const overallScore = adjustedCategoryScores.reduce((sum, cat) => sum + (cat.score * (cat.weight || 1)), 0);
        const maxPossibleScore = adjustedCategoryScores.reduce((sum, cat) => sum + (cat.maxScore * (cat.weight || 1)), 0);
        const scorePercentage = maxPossibleScore > 0 ? Math.round((overallScore / maxPossibleScore) * 100) : 0;
        // Determine readiness level
        let readinessLevel = 'MAJOR_GAPS';
