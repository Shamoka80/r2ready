import { db } from '../db';
import { scoringConfigs, answers, assessments } from '../../shared/schema';
...
export class ConfigurableScoring {
     ...
     */
    static async calculateScore(assessmentId, configId, naHandlingOverride) {
        ...
        const categoryScores = await this.calculateCategoryScores(assessmentAnswers, configWithOverride);
        const weightedScore = this.calculateWeightedScore(categoryScores, configWithOverride);
        const naCount = assessmentAnswers.filter(a => this.isNAResponse(a.value)).length;
     ...
     */
    static async getScoringConfig(configId) {
        ...
        const result = {
            ...
            appendixWeights: config.appendixWeights,
            readinessThresholds: config.readinessThresholds,
     ...
     */
    static async getDefaultScoringConfig() {
     ...
     */
    static async getAssessmentAnswers(assessmentId) {
     ...
     */
    static async calculateCategoryScores(assessmentAnswers, config) {
     ...
     */
    static getCategoryWeight(category, config) {
     ...
     */
    static calculateWeightedScore(categoryScores, config) {
        const totalWeight = categoryScores.reduce((sum, cat) => sum + cat.weight, 0);
        const weightedSum = categoryScores.reduce((sum, cat) => sum + cat.weightedContribution, 0);
        return totalWeight > 0 ? weightedSum / totalWeight : 0;
    }
    /**
     * Calculate score for an individual answer
     */
    static calculateAnswerScore(response, naHandling) {
     ...
     */
    static createEmptyResult(configName) {
        return {
            overallScore: 0,
            weightedScore: 0,
            categoryScores: [],
     ...
     */
    static async createScoringConfig(config) {
     ...
     */
    static async updateAssessmentWithScore(assessmentId, result, configId) {
