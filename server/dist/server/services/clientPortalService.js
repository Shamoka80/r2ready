...
export class ClientPortalService {
     ...
     */
    static async verifyClientAccess(userId, clientOrganizationId) {
        try {
     ...
     */
    static async getClientOrganization(userId, clientOrganizationId) {
        try {
     ...
     */
    static async getClientFacilities(userId, clientOrganizationId, filters = {}) {
        try {
            const hasAccess = await this.verifyClientAccess(userId, clientOrganizationId);
            if (!hasAccess) {
     ...
     */
    static async getClientAssessments(userId, clientOrganizationId, filters = {}) {
        try {
            const hasAccess = await this.verifyClientAccess(userId, clientOrganizationId);
            if (!hasAccess) {
            ...
            if (status && status.length > 0) {
                whereConditions.push(inArray(assessments.status, status));
            }
            if (facilityId) {
     ...
     */
    static async getClientAssessmentDetails(userId, assessmentId, clientOrganizationId) {
        try {
            const hasAccess = await this.verifyClientAccess(userId, clientOrganizationId);
            if (!hasAccess) {
                throw new Error('Access denied to client assessments');
            ...
            const [clientFacility] = await db
                .select()
                .from(clientFacilities)
                .where(and(eq(clientFacilities.clientOrganizationId, clientOrganizationId), eq(clientFacilities.id, assessment.assessment.facilityId)));
            if (!clientFacility) {
     ...
     */
    static async getPortalDashboard(userId, clientOrganizationId) {
        try {
            const hasAccess = await this.verifyClientAccess(userId, clientOrganizationId);
            if (!hasAccess) {
            ...
            const [facilitiesSummary] = await db
                .select({
                total: sql `COUNT(*)`,
                active: sql `COUNT(*) FILTER (WHERE operating_status = 'ACTIVE')`,
                inactive: sql `COUNT(*) FILTER (WHERE operating_status = 'INACTIVE')`
            })
                .from(clientFacilities)
                .where(eq(clientFacilities.clientOrganizationId, clientOrganizationId));
            ...
            if (clientFacilityIds.length > 0) {
                ...
                const [summary] = await db
                    .select({
                    total: sql `COUNT(*)`,
                    draft: sql `COUNT(*) FILTER (WHERE status = 'DRAFT')`,
                    inProgress: sql `COUNT(*) FILTER (WHERE status = 'IN_PROGRESS')`,
                    underReview: sql `COUNT(*) FILTER (WHERE status = 'UNDER_REVIEW')`,
     ...
     */
    static async updateClientOrganization(userId, clientOrganizationId, updateData) {
        try {
            const hasAccess = await this.verifyClientAccess(userId, clientOrganizationId);
            if (!hasAccess) {
            ...
            const cleanUpdates = Object.fromEntries(Object.entries(allowedUpdates).filter(([_, value]) => value !== undefined));
