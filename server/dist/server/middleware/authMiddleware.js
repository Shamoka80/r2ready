    ...
    try {
        ...
        if (payload.sessionId) {
            const sessionRecords = await db
                .select()
                .from(userSessions)
                .where(sql `${userSessions.id} = ${payload.sessionId} AND ${userSessions.userId} = ${payload.userId} AND ${userSessions.status} = 'ACTIVE'`)
                .limit(1);
        ...
        const userRecords = await db
            .select()
            .from(users)
            .where(sql `${users.id} = ${payload.userId} AND ${users.isActive} = true`)
            .limit(1);
        ...
        if (clientInfo.userAgent !== 'Unknown') {
            try {
                const deviceResult = await deviceService.registerDevice(user.id, clientInfo.userAgent, clientInfo.ipAddress, undefined, // deviceName will be auto-generated
                payload.sessionId);
