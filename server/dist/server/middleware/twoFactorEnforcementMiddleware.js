...
export class TwoFactorEnforcementMiddleware {
     ...
     */
    isConsultantAccount(user) {
        if (!user)
            return false;
        // Check if user has a consultant role
        if (user.consultantRole)
            return true;
        // Check if user belongs to a consultant tenant
        if (user.tenant?.tenantType === 'CONSULTANT')
            return true;
        return false;
    }
    /**
    ...
    isExemptRoute(req) {
        ...
        const isExempt = exemptPaths.some(exemptPath => fullPath.startsWith(exemptPath) ||
