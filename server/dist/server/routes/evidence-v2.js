...
import { assessments, questions, answers } from "../../shared/schema.js";
...
router.post('/upload/:assessmentId/:questionId', rateLimitMiddleware.evidenceUpload, requireFacilityPermissionFromAssessment('manage_assessments'), upload.array('files', 5), async (req, res) => {
    try {
        ...
        for (const file of files) {
            try {
                const result = await evidenceService.processEvidenceUpload(file, facilityId, assessmentId, questionId, req.user.id);
                results.push(result);
            }
            catch (error) {
                errors.push({
