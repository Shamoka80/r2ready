...
router.get("/conditional-rules/:intakeFormId", async (req, res) => {
    try {
...
router.get("/recommendations/:intakeFormId", async (req, res) => {
    try {
        const { intakeFormId } = req.params;
...
router.get("/workflow/:intakeFormId", async (req, res) => {
    try {
        const { intakeFormId } = req.params;
...
router.get("/pre-population", async (req, res) => {
    try {
        const prePopulationData = await SmartPrePopulationService.generatePrePopulationData(req.tenant.id, req.user.id);
        res.json({
            success: true,
...
router.post("/apply-prepopulation/:intakeFormId", async (req, res) => {
    try {
        const { intakeFormId } = req.params;
        ...
        const result = await SmartPrePopulationService.applySmartPrePopulation(intakeFormId, validatedData.prePopulationData, validatedData.userPreferences);
        res.json({
...
router.post("/evaluate-dependencies", async (req, res) => {
    try {
        ...
        const conditionalRules = await ConditionalQuestionService.evaluateConditionalRules(validatedData.intakeFormId);
        res.json({
            success: true,
            conditionalRules,
            meta: {
