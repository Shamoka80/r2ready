...
router.get('/:id/scoring', requireFacilityPermissionFromAssessment('view_reports'), async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/:id/scoring/refresh', requireFacilityPermissionFromAssessment('manage_assessments'), async (req, res) => {
    try {
        const { id } = req.params;
...
export async function calculateAssessmentScore(assessmentId, intakeScope = null) {
    ...
    const questionsWithAnswers = await db.select({
        ...
        .leftJoin(answers, and(eq(answers.questionId, questions.id), eq(answers.assessmentId, assessmentId)))
    ...
    let intakeBasedScoring = undefined;
    if (intakeScope) {
        intakeBasedScoring = {
...
function determineCategoryKey(question) {
...
function calculateCategoryScore(categoryKey, questions, intakeScope) {
...
function determineComplianceStatus(scorePercentage, categoryScores, criticalIssues) {
...
function generateRecommendations(categoryScores, intakeScope) {
    ...
    if (intakeScope?.applicableRecCodes) {
        ...
        const relevantAppendices = appendixCategories.filter(cat => intakeScope.applicableRecCodes.includes(cat.category));
...
function determineReadinessLevel(scorePercentage, criticalIssuesCount, categoryScores) {
...
function calculateAuditSuccessProbability(scorePercentage, categoryScores, intakeScope) {
...
function calculateWeightAdjustments(intakeScope) {
    const adjustments = {};
    if (intakeScope?.applicableRecCodes) {
...
function calculateScopeSpecificScore(categoryScores, intakeScope) {
    if (!intakeScope?.applicableRecCodes) {
        return 0;
    }
    // Calculate score for only the categories relevant to the intake scope
    const relevantCategories = categoryScores.filter(cat => intakeScope.applicableRecCodes.includes(cat.category) ||
