...
const onboardingAnalyticsStore = new Map();
...
router.get('/onboarding/summary', async (req, res) => {
    try {
        ...
        const avgStepTimes = Object.fromEntries(Object.entries(stepTimes).map(([step, times]) => [
            step,
...
router.get('/compliance/kpis', async (req, res) => {
    try {
        ...
        await ObservabilityService.recordMetric('compliance_kpis_request', 1, 'count', {
            service: 'analytics',
...
router.get('/compliance/predictive-insights', async (req, res) => {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(401).json({ error: 'Authentication required' });
        ...
        await ObservabilityService.recordMetric('predictive_insights_request', 1, 'count', {
            service: 'analytics',
...
router.get('/compliance/dashboard-metrics', async (req, res) => {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(401).json({ error: 'Authentication required' });
        ...
        await ObservabilityService.recordMetric('dashboard_metrics_request', 1, 'count', {
            service: 'analytics',
...
router.get('/compliance', AuthService.authMiddleware, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const timeRange = req.query.timeRange || '30d';
...
router.get('/user-activity', AuthService.authMiddleware, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const timeRange = req.query.timeRange || '24h';
...
router.get('/assessments', AuthService.authMiddleware, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const timeRange = req.query.timeRange || '30d';
        ...
        const scoreDistribution = await db
            ...
            .from(assessments)
            .where(and(eq(assessments.tenantId, tenantId), isNotNull(assessments.overallScore), eq(assessments.status, 'COMPLETED')));
        ...
        await ObservabilityService.recordMetric('analytics_view', 1, 'count', {
            service: 'analytics',
...
function generateTrendData(timeRange, granularity = 'daily') {
...
function generateTimelineMilestones(scoringData) {
    const milestones = [
        {
...
function generateGapAnalysis(categoryScores) {
...
function generateCategoryRecommendations(category, gap) {
...
function generatePredictiveInsights(scoringData, trends) {
    const currentScore = scoringData.scorePercentage;
    const criticalIssuesCount = scoringData.criticalIssues.length;
    const questionsRemaining = scoringData.categoryScores.reduce((total, cat) => total + (cat.totalQuestions - cat.questionsAnswered), 0);
    ...
    if (criticalIssuesCount > 0)
        certificationProbability -= (criticalIssuesCount * 10);
    if (scoringData.readinessLevel === 'AUDIT_READY')
        certificationProbability += 10;
...
async function calculateAverageCompletionTime(tenantId) {
    const completedAssessments = await db
        ...
        .from(assessments)
        .where(and(eq(assessments.tenantId, tenantId), eq(assessments.status, 'COMPLETED'), isNotNull(assessments.createdAt), isNotNull(assessments.completedAt)));
    if (completedAssessments.length === 0) {
        return 'N/A';
    }
    const totalTimeInMs = completedAssessments.reduce((sum, assessment) => {
        if (!assessment.createdAt || !assessment.completedAt)
            return sum;
