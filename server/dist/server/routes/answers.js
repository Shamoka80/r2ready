...
router.post("/:assessmentId/answers", async (req, res) => {
    try {
        ...
        const assessment = await db.query.assessments.findFirst({
            where: and(eq(assessments.id, assessmentId), eq(assessments.tenantId, req.tenant.id)),
        });
        if (!assessment) {
        ...
        const existingAnswer = await db.query.answers.findFirst({
            where: and(eq(answers.assessmentId, assessmentId), eq(answers.questionId, data.questionId)),
        });
...
router.post("/:assessmentId/answers/batch", async (req, res) => {
    try {
        const { assessmentId } = req.params;
        ...
        for (const answerItem of answerData) {
            ...
            const existingAnswer = await db.query.answers.findFirst({
                where: and(eq(answers.assessmentId, assessmentId), eq(answers.questionId, answerItem.questionId)),
            });
            if (existingAnswer) {
                // Update existing
...
router.get("/:assessmentId/answers", async (req, res) => {
    try {
        const { assessmentId } = req.params;
        // Verify assessment ownership
        const assessment = await db.query.assessments.findFirst({
...
router.get("/:assessmentId/answers/:questionId", async (req, res) => {
    try {
...
router.delete("/:assessmentId/answers/:questionId", async (req, res) => {
    try {
        const { assessmentId, questionId } = req.params;
        // Verify assessment ownership
        const assessment = await db.query.assessments.findFirst({
...
async function updateAssessmentProgress(assessmentId) {
    try {
        ...
        const [scoreData] = await db
            ...
            totalScore: sql `sum(${answers.score})`,
            maxScore: sql `sum(${answers.maxScore})`
        })
            .from(answers)
            .where(eq(answers.assessmentId, assessmentId));
...
async function trackAssessmentActivity(assessmentId, userId, tenantId, activityType) {
    try {
        // Find or create current session
        const activeSession = await db.query.assessmentSessions.findFirst({
            where: and(eq(assessmentSessions.assessmentId, assessmentId), eq(assessmentSessions.userId, userId), isNull(assessmentSessions.sessionEnd)),
        });
        if (activeSession) {
