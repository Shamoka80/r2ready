...
router.post("/", async (req, res) => {
    ...
    `, [
        ...
        // Log audit event
        await AuthService.logAuditEvent(req.tenant.id, req.user.id, 'ASSESSMENT_TEMPLATE_CREATED', 'assessment_template', templateId, undefined, { name: data.name, facilityTypes: data.facilityTypes });
...
router.put("/:id", async (req, res) => {
    ...
    `, updateParams);
        // Log audit event
        await AuthService.logAuditEvent(req.tenant.id, req.user.id, 'ASSESSMENT_TEMPLATE_UPDATED', 'assessment_template', id, existing.rows[0], data);
