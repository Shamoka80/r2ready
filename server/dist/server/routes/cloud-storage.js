 ...
 */
router.post('/configure', AuthService.authMiddleware, requireFlag('aws_s3_storage'), rateLimitMiddleware.general, async (req, res) => {
    try {
 ...
 */
router.post('/upload', AuthService.authMiddleware, rateLimitMiddleware.general, upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
        ...
        const result = await cloudStorageService.uploadFile(req.user.tenantId, req.file.buffer, options);
        if (!result.success) {
            return res.status(400).json({ error: result.error });
        }
        res.json({
 ...
 */
router.get('/download/:fileId', AuthService.authMiddleware, async (req, res) => {
    try {
 ...
 */
router.delete('/:fileId', AuthService.authMiddleware, async (req, res) => {
    try {
        const { fileId } = req.params;
 ...
 */
router.get('/quota', AuthService.authMiddleware, async (req, res) => {
    try {
