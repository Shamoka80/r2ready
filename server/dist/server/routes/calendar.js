...
router.get("/events", rateLimitMiddleware.general, async (req, res) => {
    try {
...
router.post("/events", rateLimitMiddleware.general, async (req, res) => {
    try {
...
router.put("/events/:id", rateLimitMiddleware.general, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        await AuthService.logAuditEvent(req.tenant.id, req.user.id, 'CALENDAR_EVENT_UPDATED', 'calendar_event', id, undefined, updateData);
        res.json({
            success: true,
...
router.get("/upcoming-deadlines", rateLimitMiddleware.general, async (req, res) => {
    try {
...
router.post("/generate-compliance-calendar", rateLimitMiddleware.general, async (req, res) => {
    try {
        const { assessmentId } = z.object({
            assessmentId: z.string().uuid()
        }).parse(req.body);
        const events = await calendarService.generateComplianceCalendar(req.tenant.id, assessmentId);
        await AuthService.logAuditEvent(req.tenant.id, req.user.id, 'COMPLIANCE_CALENDAR_GENERATED', 'assessment', assessmentId, undefined, { eventsCount: events.length });
        res.json({
            success: true,
...
router.get("/dashboard", rateLimitMiddleware.general, async (req, res) => {
    try {
