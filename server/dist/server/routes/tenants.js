...
router.get('/branding', AuthService.authMiddleware, async (req, res) => {
    try {
...
router.patch('/branding', AuthService.authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenant?.id;
        if (!tenantId) {
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        if (brandColorPrimary !== undefined)
            updateData.brandColorPrimary = brandColorPrimary;
        if (brandColorSecondary !== undefined)
            updateData.brandColorSecondary = brandColorSecondary;
...
router.post('/upload-logo', AuthService.authMiddleware, (req, res) => {
