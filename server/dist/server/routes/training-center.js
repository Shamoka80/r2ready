...
router.get('/knowledge-base', async (req, res) => {
    try {
        ...
        const articles = await TrainingCenterService.getKnowledgeBase(search, category);
        res.json({
            success: true,
            articles,
...
router.get('/certification-prep', async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, error: 'Authentication required' });
...
router.post('/modules/:moduleId/certificate', async (req, res) => {
    try {
        const userId = req.user?.id;
        const { moduleId } = req.params;
        if (!userId) {
...
router.post('/tutorials/:tutorialId/steps/:stepId', async (req, res) => {
    try {
        const userId = req.user?.id;
...
router.get('/progress/comprehensive', async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, error: 'Authentication required' });
