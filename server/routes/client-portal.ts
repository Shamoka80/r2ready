...
const requireBusinessUser = (req: AuthenticatedRequest, res: any, next: any) => {
 ...
 */
router.get('/dashboard/:clientOrgId', async (req: AuthenticatedRequest, res) => {
  try {
    const { clientOrgId } = req.params;

    const dashboardData = await ClientPortalService.getPortalDashboard(
      req.user!.id,
      clientOrgId
    );
 ...
 */
router.get('/organization/:clientOrgId', async (req: AuthenticatedRequest, res) => {
  try {
    const { clientOrgId } = req.params;

    const organization = await ClientPortalService.getClientOrganization(
      req.user!.id,
      clientOrgId
    );
 ...
 */
router.put('/organization/:clientOrgId', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const result = await ClientPortalService.updateClientOrganization(
      req.user!.id,
      clientOrgId,
      validatedData
    );
 ...
 */
router.get('/facilities/:clientOrgId', async (req: AuthenticatedRequest, res) => {
  try {
    const { clientOrgId } = req.params;
    const {
      operatingStatus,
    ...
    const filters = {
      operatingStatus: operatingStatus as string,
      limit: parseInt(limit as string),
      offset: parseInt(offset as string)
    };

    const facilities = await ClientPortalService.getClientFacilities(
      req.user!.id,
      clientOrgId,
      filters
    );
 ...
 */
router.get('/assessments/:clientOrgId', async (req: AuthenticatedRequest, res) => {
  try {
    const { clientOrgId } = req.params;
    const {
      status,
    ...
    const assessments = await ClientPortalService.getClientAssessments(
      req.user!.id,
      clientOrgId,
      filters
    );
 ...
 */
router.get('/assessments/:clientOrgId/:assessmentId', async (req: AuthenticatedRequest, res) => {
  try {
    const { clientOrgId, assessmentId } = req.params;

    const assessmentDetails = await ClientPortalService.getClientAssessmentDetails(
      req.user!.id,
      assessmentId,
      clientOrgId
    );
