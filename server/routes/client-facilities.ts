  ...
  try {
    ...
    const organization = await db.query.clientOrganizations.findFirst({
      where: and(
        ...
        eq(clientOrganizations.tenantId, req.tenant!.id)
      ),
    });
...
router.get("/:id", async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const facility = await db.query.clientFacilities.findFirst({
      where: and(
        eq(clientFacilities.id, id),
        eq(clientFacilities.tenantId, req.tenant!.id)
      ),
      with: {
