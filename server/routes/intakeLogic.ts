...
export interface RECMappingInfo {
  applicableRecCodes: string[];
  scopeStatement: string;
  requiredAppendices: string[];
  complexityFactors: {
    facilityComplexity: number;
...
export class IntakeProcessor {
   ...
   */
  static async generateAssessmentScope(intakeFormId: string): Promise<AssessmentScope> {
   ...
   */
  static async determineApplicableRECs(intakeForm: any, allRecMappings: any[]): Promise<string[]> {
    const applicableRecs = new Set<string>();
   ...
   */
  static async evaluateRECApplicability(intakeForm: any, recMapping: any): Promise<boolean> {
   ...
   */
  static evaluateTriggerConditions(intakeForm: any, triggers: any): boolean {
    for (const [triggerName, condition] of Object.entries(triggers)) {
      if (this.evaluateCondition(intakeForm, condition as string, triggerName)) {
        return true;
      }
    }
    return false;
   ...
   */
  static evaluateCondition(intakeForm: any, condition: string, triggerName: string): boolean {
    try {
      ...
      if (condition.includes('data_destruction_activities === true')) {
        return intakeForm.processingActivities?.some((activity: string) => 
          activity.toLowerCase().includes('data destruction') || 
      ...
      if (condition.includes('handles_mercury') || condition.includes('mercury_containing')) {
        return intakeForm.focusMaterials?.some((material: string) => 
          material.toLowerCase().includes('mercury')
        );
      }

      if (condition.includes('handles_lead') || condition.includes('lead_containing')) {
        return intakeForm.focusMaterials?.some((material: string) => 
          material.toLowerCase().includes('lead') || material.toLowerCase().includes('crt')
        );
      }
   ...
   */
  static evaluateDefaultApplicability(intakeForm: any, recCode: string): boolean {
    ...
    if (recCode.startsWith('DATA-')) {
      return intakeForm.processingActivities?.some((activity: string) =>
        activity.toLowerCase().includes('data')
      );
    }
   ...
   */
  static generateScopeStatement(intakeForm: any, applicableRecCodes: string[]): string {
    const parts = [];
    ...
    // Data security scope
    const hasDataActivities = intakeForm.processingActivities?.some((activity: string) =>
      activity.toLowerCase().includes('data')
    );
   ...
   */
  static determineRequiredAppendices(intakeForm: any): string[] {
   ...
   */
  static calculateComplexityFactors(intakeForm: any, applicableRecCodes: string[]): any {
    const factors = {
   ...
   */
  static estimateAuditDays(intakeForm: any, complexityFactors: any): number {
   ...
   */
  static identifyCriticalRequirements(intakeForm: any, applicableRecCodes: string[]): string[] {
    const critical = [];
   ...
   */
  static validateIntakeCompleteness(intakeData: any): ValidationResult {
   ...
   */
  static determineRequiredAppendicesFromIntake(intakeData: any): string[] {
   ...
   */
  static async filterQuestionsForAssessment(intakeFormId: string, assessmentId: string) {
