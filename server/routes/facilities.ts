      ...
      facilities.map(async (facility) => {
        const assessmentCount = await db
          ...
          .where(and(
            ...
            eq(assessments.facilityId, facility.id)
          ));
...
router.post("/", async (req: AuthenticatedRequest, res) => {
  try {
    ...
    await AuthService.logAuditEvent(
      ...
      newFacility.id,
      undefined,
      { name: newFacility.name, isPrimary }
    );
...
router.delete("/:id", async (req: AuthenticatedRequest, res) => {
  try {
    ...
    await AuthService.logAuditEvent(
      ...
      'facility',
      id,
      { name: existingFacility.name, isActive: true },
      { name: existingFacility.name, isActive: false }
    );
...
router.post("/:id/assign-user", async (req: AuthenticatedRequest, res) => {
  try {
    ...
    `);
    const existingAssignment = (existingAssignmentResult as any).rows || existingAssignmentResult;
    ...
    const assignmentResult = await db.execute(sql`
      ...
      RETURNING *
    `);
    const assignment = (assignmentResult as any).rows || assignmentResult;

    // Log audit event
    await AuthService.logAuditEvent(
      req.tenant!.id,
      req.user!.id,
      'FACILITY_USER_ASSIGNED',
      'facility',
      facilityId,
      undefined,
      { userId, role, facilityName: facility.name }
    );
...
router.post("/:id/assign-user-by-email", async (req: AuthenticatedRequest, res) => {
  try {
    const { id: facilityId } = req.params;
    ...
    `);
    const existingAssignment = (existingAssignmentResult2 as any).rows || existingAssignmentResult2;
    ...
    const assignmentResult2 = await db.execute(sql`
      ...
      RETURNING *
    `);
    const assignment = (assignmentResult2 as any).rows || assignmentResult2;
...
router.get("/:id/users", async (req: AuthenticatedRequest, res) => {
  try {
    const { id: facilityId } = req.params;
...
router.delete("/:id/users/:userId", async (req: AuthenticatedRequest, res) => {
  try {
...
router.get("/user/facility-scope", async (req: AuthenticatedRequest, res) => {
  try {
    ...
    `);
    const userFacilityAssignments = (userFacilityAssignmentsResult as any).rows || userFacilityAssignmentsResult;
    ...
    const assignedFacilities = userFacilityAssignments.length > 0 
      ? userFacilityAssignments.map((assignment: any) => assignment.facilityId)
