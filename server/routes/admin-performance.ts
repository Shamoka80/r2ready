...
router.get('/', authenticateUser as any, async (req: Request, res: Response) => {
  try {
    const userRole = (req.user as any)?.businessRole || (req.user as any)?.consultantRole;
...
router.get('/index-health', authenticateUser as any, async (req: Request, res: Response) => {
  try {
    const userRole = (req.user as any)?.businessRole || (req.user as any)?.consultantRole;
...
function redactSensitiveData(query: string): string {
  let redacted = query;
