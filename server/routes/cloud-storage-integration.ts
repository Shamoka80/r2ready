...
import { cloudStorageService } from '../services/cloudStorageIntegrationService';
...
router.get('/providers', authenticateUser as any, async (req, res) => {
  try {
...
router.post('/upload/:provider', authenticateUser as any, upload.single('file'), async (req, res) => {
  try {
    ...
    switch (provider) {
      case 'google_drive':
        result = await cloudStorageService.uploadToGoogleDrive(
          file.buffer, 
          file.originalname, 
          file.mimetype
        );
...
router.get('/download/:provider/:fileId', authenticateUser as any, async (req, res) => {
  try {
...
router.get('/quota/:provider', authenticateUser as any, async (req, res) => {
  try {
    const { provider } = req.params;
...
router.post('/upload/bulk', authenticateUser as any, upload.array('files', 10), async (req, res) => {
  try {
