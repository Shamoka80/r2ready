...
router.get('/portfolio', async (req: Request, res: Response) => {
  try {
    ...
    const portfolio = await ConsultantFeaturesService.getClientPortfolio(userId, clientId as string);
...
router.post('/reports/white-label', async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
...
router.get('/analytics', async (req: Request, res: Response) => {
  try {
    ...
    const timeRange = {
      startDate: startDate ? new Date(startDate as string) : new Date(Date.now() - 90 * 24 * 60 * 60 * 1000), // 90 days ago
...
router.get('/branding/options', async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ success: false, error: 'Authentication required' });
...
router.post('/branding/update', async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
...
router.get('/branding/templates', async (req: Request, res: Response) => {
  try {
    const templates = [
      {
