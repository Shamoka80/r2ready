...
import { validation, securitySchemas } from '../middleware/validationMiddleware.js';
...
export async function validateRoleAssignment(
  ...
  if (assignerIsBusiness && targetIsBusiness) {
    const assignerLevel = businessRoleHierarchy[assignerRole as keyof typeof businessRoleHierarchy];
    const targetLevel = businessRoleHierarchy[targetRole as keyof typeof businessRoleHierarchy];
  ...
  if (assignerIsConsultant && targetIsConsultant) {
    const assignerLevel = consultantRoleHierarchy[assignerRole as keyof typeof consultantRoleHierarchy];
    const targetLevel = consultantRoleHierarchy[targetRole as keyof typeof consultantRoleHierarchy];
    // Can only assign roles at or below your level
    return assignerLevel >= targetLevel;
  }
...
router.get('/system-status', async (req, res) => {
  try {
    ...
    const recMappings = await db.select({ count: sql<number>`count(*)` }).from(questionMapping);
...
router.post('/register-tenant', rateLimitMiddleware.register, blockTestUserRegistration, async (req, res) => {
  try {
    ...
    if (existingUser) {
      ...
      const isStuckInIncompleteState = 
        existingUser.setupStatus === 'email_pending' || 
      ...
      await AuthService.logAuditEvent(
        ...
        existingUser.id,
        'REGISTRATION_RECOVERY_RESTART',
        'user',
        existingUser.id,
        { email: data.ownerEmail, previousStatus: existingUser.setupStatus }
      );
    ...
    const { tenant, user } = await AuthService.createTenant(
      data.tenantName,
      data.tenantType,
      data.ownerEmail,
      data.ownerFirstName,
    ...
    if (emailVerificationEnabled) {
      ...
      await AuthService.logAuditEvent(
        ...
        'user',
        user.id,
        { email: data.ownerEmail, tenantName: data.tenantName }
      );
      ...
      const permissions = await AuthService.getUserPermissions(
        data.tenantType === 'BUSINESS' ? 'business_owner' : 'consultant_owner'
      );
 ...
 */
router.post('/logout', AuthService.authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    if (req.session) {
 ...
 */
router.get('/me', AuthService.authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    const permissions = await AuthService.getUserPermissions(
      req.user!.businessRole || req.user!.consultantRole!
    );
 ...
 */
router.post('/invite-user', AuthService.authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
 ...
 */
router.post('/assign-role', AuthService.authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
 ...
 */
router.post('/update-setup-status', AuthService.authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
 ...
 */
router.patch('/account-type', AuthService.authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
 ...
 */
router.post('/set-password', strictRateLimit.passwordChange, AuthService.authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
 ...
 */
router.get('/users', AuthService.authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
 ...
 */
router.get('/audit-logs', AuthService.authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const logs = await db
      .select()
      .from(auditLog as any)
      .where(sql`${(auditLog as any).tenantId} = ${req.tenant!.id}`)
      .orderBy(sql`${(auditLog as any).timestamp} DESC`)
      .limit(limit)
      .offset(offset);
...
router.post('/complete-registration', rateLimitMiddleware.login, async (req, res) => {
  try {
    ...
    if (!user && session.metadata?.registrationFlow === 'true' && process.env.NODE_ENV !== 'production') {
      ...
      // Create tenant
      const tenantResult = await db.insert(tenants as any).values({
        name: companyName,
        tenantType: tenantType === 'CONSULTANT' ? 'CONSULTANT' : 'BUSINESS',
        licenseStatus: 'active',
        isActive: true,
      }).returning();
      const newTenant = (tenantResult as any)[0];

      // Create user
      const userResult = await db.insert(users as any).values({
      ...
      const newUser = (userResult as any)[0];
      ...
      if (licenseId) {
        const licenseConfig = licenseConfigs[licenseId as keyof typeof licenseConfigs];
        if (licenseConfig) {
          ...
          const [newLicense] = await db.insert(licenses).values({
            ...
            isActive: true,
            maxFacilities: (licenseConfig as any).maxFacilities ?? null,
            maxSeats: (licenseConfig as any).maxSeats ?? null,
            maxClients: (licenseConfig as any).maxClients ?? null,
            supportHours: (licenseConfig as any).supportHours ?? null,
            supportTier: (licenseConfig as any).supportTier ?? null,
    ...
    const { session: userSession, token } = await AuthService.createSession(
      user.id,
      user.tenantId,
      ipAddress,
      userAgent
    ...
    res.json({
      ...
      tenant: {
        id: (user as any).tenant?.id || user.tenantId,
        name: (user as any).tenant?.name || 'Unknown',
        type: (user as any).tenant?.tenantType || 'BUSINESS',
        licenseStatus: (user as any).tenant?.licenseStatus || 'active',
      },
      token,
      permissions,
 ...
 */
router.post('/auto-provision-test-license', AuthService.authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const existingLicense = await db.query.licenses.findFirst({
      where: (licenses, { eq, and }) => and(
        eq(licenses.tenantId, req.tenant!.id),
    ...
    // Create test license
    const licenseResult = await db.insert(licenses as any).values({
    ...
    const newLicense = Array.isArray(licenseResult) ? licenseResult[0] : (licenseResult as any).rows[0];

    // Create license event
    await db.insert(licenseEvents as any).values({
      tenantId: req.tenant!.id,
      licenseId: newLicense.id,
    ...
    await db.update(tenants as any)
      .set({
        licenseStatus: 'active',
        updatedAt: new Date()
      })
...
router.post('/send-verification-email', strictRateLimit.passwordChange, async (req, res) => {
  try {
    ...
    await AuthService.logAuditEvent(
      user.tenantId,
      user.id,
      isStuckInIncompleteState ? 'VERIFICATION_EMAIL_RESENT_RECOVERY' : 'VERIFICATION_EMAIL_SENT',
      'user',
      user.id,
...
router.post('/verify-email-code',
  rateLimitMiddleware.login, // Strict rate limiting for brute-force protection on auth codes
  async (req, res) => {
  try {
