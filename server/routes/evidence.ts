...
router.get('/:assessmentId/:questionId', 
  ...
  try {
    ...
    if (cursor && cursor.timestamp) {
      ...
      if (direction === 'forward') {
        ...
        whereCondition = and(
          whereCondition,
          or(
            lt(evidenceObjects.createdAt, cursorTimestamp),
            and(
              eq(evidenceObjects.createdAt, cursorTimestamp),
              gt(evidenceObjects.id, cursor.id)
            )
          )
        )!;
      } else {
        ...
        whereCondition = and(
          whereCondition,
          or(
            gt(evidenceObjects.createdAt, cursorTimestamp),
            and(
              eq(evidenceObjects.createdAt, cursorTimestamp),
              lt(evidenceObjects.id, cursor.id)
            )
          )
        )!;
      }
    ...
    .orderBy(
      direction === 'forward' ? desc(evidenceObjects.createdAt) : evidenceObjects.createdAt,
      direction === 'forward' ? evidenceObjects.id : desc(evidenceObjects.id)
    )
    .limit(limit + 1);
