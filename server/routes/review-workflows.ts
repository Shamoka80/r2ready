 ...
 */
router.post('/', rateLimitMiddleware.general, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const workflow = await ReviewWorkflowService.createReviewWorkflow(
      validatedData.assessmentId,
      validatedData.clientOrganizationId,
      req.user!.tenantId,
      req.user!.id,
      {
...
router.get('/', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const workflows = await ReviewWorkflowService.getConsultantWorkflows(
      req.user!.tenantId,
      filters
    );
...
router.get('/assigned', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const workflows = await ReviewWorkflowService.getReviewerWorkflows(
      req.user!.id,
      req.user!.tenantId,
      filters
    );
 ...
 */
router.put('/:id/assign', rateLimitMiddleware.general, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    await ReviewWorkflowService.assignReviewer(
      workflowId,
      req.user!.id,
 ...
 */
router.put('/:id/start', rateLimitMiddleware.general, async (req: AuthenticatedRequest, res) => {
  try {
    const { id: workflowId } = req.params;
    const { comments } = req.body;

    await ReviewWorkflowService.startReview(
      workflowId,
      req.user!.id,
      comments
    );
 ...
 */
router.put('/:id/complete', rateLimitMiddleware.general, async (req: AuthenticatedRequest, res) => {
  try {
    const { id: workflowId } = req.params;
    const validatedData = completeReviewSchema.parse(req.body);

    await ReviewWorkflowService.completeReview(
      workflowId,
      req.user!.id,
 ...
 */
router.get('/:id/decisions', async (req: AuthenticatedRequest, res) => {
  try {
    const { id: workflowId } = req.params;

    const decisionLog = await ReviewWorkflowService.getDecisionLog(
      workflowId,
      req.user!.tenantId
    );
 ...
 */
router.post('/invitations', rateLimitMiddleware.general, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const result = await ReviewWorkflowService.createClientInvitation(
      validatedData.clientOrganizationId,
      req.user!.tenantId,
      req.user!.id,
      validatedData.invitedEmail,
      validatedData.invitedRole,
      {
...
router.get('/invitations', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const invitations = await ReviewWorkflowService.getClientInvitations(
      req.user!.tenantId,
      filters
    );
