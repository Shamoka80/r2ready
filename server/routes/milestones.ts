...
router.post("/", async (req: AuthenticatedRequest, res) => {
  try {
    ...
    await AuthService.logAuditEvent(
      ...
      milestoneId,
      undefined,
      { name: data.name, category: data.category, assessmentId: data.assessmentId }
    );
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (value !== undefined) {
        if (key === 'targetDate' || key === 'completedDate') {
          ...
          updateParams.push(value ? new Date(value as string) : null);
...
router.get("/dashboard/:assessmentId", async (req: AuthenticatedRequest, res) => {
  try {
    const { assessmentId } = req.params;
