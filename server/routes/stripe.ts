...
router.post("/create-checkout-session",
  rateLimitMiddleware.stripeCheckout,
  async (req: AuthenticatedRequest, res) => {
    try {
      if (!stripe) {
        return res.status(503).json({ 
      ...
      const licenseConfigs: Record<string, { price: number; name: string; description?: string }> = {
        // Business tiers
      ...
      const licenseIdToTierMap: Record<string, string> = {
...
router.post("/session/:sessionId/activate",
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const validBaseTiers: ValidTier[] = [
      ...
      if (isAddon) {
        ...
        const existingLicense = await db.query.licenses.findFirst({
          where: and(
            eq(licenses.tenantId, licenseTenantId),
      ...
      if (!validBaseTiers.includes(tier as ValidTier)) {
