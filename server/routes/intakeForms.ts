...
router.get("/:id", async (req: AuthenticatedRequest, res) => {
  try {
    ...
    res.json({
      ...intakeForm,
      completionPercentage,
      requiredQuestionsAnswered: intakeForm.intakeAnswers.filter(a => 
        allIntakeQuestions.find(q => q.id === a.intakeQuestionId)?.required
      ).length,
...
router.post("/:id/answers", async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
    ...
    const existingAnswer = await db.query.intakeAnswers.findFirst({
      where: and(
        eq(intakeAnswers.intakeFormId, id),
        eq(intakeAnswers.intakeQuestionId, data.intakeQuestionId)
      ),
    });
...
router.get("/:id/answers", async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
...
router.post("/:id/facilities", async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
...
router.post("/:id/submit", async (req: AuthenticatedRequest, res) => {
  try {
    ...
    await AuthService.logAuditEvent(
      ...
      assessment.id,
      undefined,
      { intakeFormId: id, title: assessment.title }
    );
...
router.get("/:id/validation", async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
