...
import { AuthService, requireFacilityPermission, getUserFacilityPermissions, type AuthenticatedRequest } from "../services/authService";
...
router.get('/facilities/:facilityId/users', requireFacilityPermission('manage_users'), async (req: AuthenticatedRequest, res) => {
  try {
...
router.post('/facilities/:facilityId/assign-user', requireFacilityPermission('manage_users'), async (req: AuthenticatedRequest, res) => {
  try {
    const { facilityId } = req.params;
...
router.put('/facilities/:facilityId/users/:userId', requireFacilityPermission('manage_users'), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const currentAssignment = existingAssignment.rows[0] as any;
    ...
    await AuthService.logAuditEvent(
      req.tenant!.id,
      req.user!.id,
      'FACILITY_USER_PERMISSIONS_UPDATED',
      'facility',
      facilityId,
      {
...
router.delete('/facilities/:facilityId/users/:userId', requireFacilityPermission('manage_users'), async (req: AuthenticatedRequest, res) => {
  try {
    const { facilityId, userId } = req.params;
    ...
    const assignment = existingAssignment.rows[0] as any;
...
router.get('/users/:userId/facilities', async (req: AuthenticatedRequest, res) => {
  try {
...
router.get('/my-facilities', async (req: AuthenticatedRequest, res) => {
  try {
