...
import { db } from '../db';
import { userCloudStorageConnections, type CloudStorageProvider } from '@shared/schema';
...
export class OAuthService {
  private configs: Map<CloudStorageProvider, OAuthConfig> = new Map();
   ...
   */
  getAuthorizationUrl(provider: CloudStorageProvider, userId: string): string {
   ...
   */
  async refreshAccessToken(provider: CloudStorageProvider, refreshToken: string): Promise<OAuthTokens> {
    const config = this.configs.get(provider);
    if (!config) {
      throw new Error(`OAuth not configured for provider: ${provider}`);
    }
   ...
   */
  private async getUserInfo(provider: CloudStorageProvider, accessToken: string): Promise<{ id: string; email: string }> {
    let url: string;
  ...
  async storeTokens(
    ...
    const existing: any = await db
      .select()
      .from(userCloudStorageConnections as any)
      .where(
        and(
          eq(userCloudStorageConnections.userId, userId),
          eq(userCloudStorageConnections.provider, provider)
        ) as any
      )
      .limit(1);
    ...
    if (existing.length > 0) {
      ...
      await db
        .update(userCloudStorageConnections as any)
        .set({
        ...
        .where(eq(userCloudStorageConnections.id, existing[0].id) as any);
    } else {
      ...
      await db.insert(userCloudStorageConnections as any).values({
        userId,
        provider,
        accessToken: tokens.accessToken,
  ...
  private async getUserConnectionCount(userId: string): Promise<number> {
    const connections: any = await db
      .select()
      .from(userCloudStorageConnections as any)
      .where(eq(userCloudStorageConnections.userId, userId) as any);
   ...
   */
  private generateState(userId: string, provider: CloudStorageProvider): string {
   ...
   */
  private verifyState(state: string, expectedProvider: CloudStorageProvider): { userId: string; provider: CloudStorageProvider } {
    try {
   ...
   */
  isProviderConfigured(provider: CloudStorageProvider): boolean {
   ...
   */
  getConfiguredProviders(): CloudStorageProvider[] {
