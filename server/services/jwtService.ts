...
export class JWTService {
  ...
  // Token configuration
  private readonly accessTokenSecret = this.getRequiredSecret('JWT_ACCESS_SECRET');
  private readonly refreshTokenSecret = this.getRequiredSecret('JWT_REFRESH_SECRET');
  private readonly accessTokenExpiry = '15m'; // Short-lived access tokens
  private readonly refreshTokenExpiry = '7d'; // Refresh tokens last 1 week
  
  /**
   * Get required secret from environment variables with validation
   */
  private getRequiredSecret(envVarName: string): string {
  ...
  async verifyRefreshToken(token: string): Promise<{
    ...
    try {
      ...
      const tokenRecords = await db
        ...
        .from(refreshTokens)
        .where(and(
          eq(refreshTokens.jti, typedPayload.jti),
          eq(refreshTokens.userId, typedPayload.userId),
  ...
  async refreshAccessToken(
    ...
    if (!verification.valid || !verification.payload || !verification.tokenRecord) {
      await this.logSecurityEvent(
        ...
        verification.payload?.userId || 'unknown',
  ...
  async revokeRefreshToken(
    ...
    try {
      ...
      await this.logSecurityEvent(
        ...
        deviceId || result[0].deviceId || undefined,
        { reason, tokenId },
        ipAddress,
        userAgent
      );
  ...
  async revokeAllUserTokens(
    ...
    try {
      ...
      await this.logSecurityEvent(
        ...
        undefined,
        undefined,
        { reason, revokedCount: result.length, excludeSessionId },
        ipAddress,
        userAgent
      );
