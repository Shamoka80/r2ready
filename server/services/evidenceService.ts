...
export class EvidenceService {
  private readonly config: EvidenceValidationConfig;
  ...
  validateMimeType(file: Express.Multer.File): { isValid: boolean; warning?: string } {
  ...
  async validateFileSize(file: Express.Multer.File): Promise<{ isValid: boolean; warning?: string }> {
  ...
  async validateFileName(file: Express.Multer.File): Promise<{ isValid: boolean; warning?: string }> {
  ...
  async performDeepContentScan(filePath: string): Promise<{ isValid: boolean; warnings: string[]; threats: string[] }> {
  ...
  async generateImmutableStorageUri(facilityId: string, assessmentId: string, questionId: string, filename: string): Promise<string> {
  ...
  async moveToImmutableStorage(sourcePath: string, storageUri: string): Promise<string> {
  ...
  private async queueAntivirusScan(evidenceId: string, filePath: string): Promise<void> {
  ...
  private async simulateAntivirusScan(filePath: string): Promise<{ status: 'clean' | 'infected' | 'error'; details: any }> {
  ...
  private async updateScanStatus(evidenceId: string, status: 'clean' | 'infected' | 'error', details: any): Promise<void> {
  ...
  private async quarantineFile(evidenceId: string): Promise<void> {
    try {
      ...
      if (evidence && evidence.storageUri) {
        ...
        await db.update(evidenceObjects)
          .set({
            ...
            scanResults: {
              ...(evidence.scanResults as Record<string, any> || {}),
  ...
  async getEvidenceAuditLog(evidenceId: string): Promise<any> {
    const [evidence] = await db.select()
      .from(evidenceObjects)
      .where(eq(evidenceObjects.id, evidenceId));
    ...
    return {
      ...
      auditTrail: [
        ...
        ...(evidence.scanResults && typeof evidence.scanResults === 'object' && (evidence.scanResults as any).scannedAt ? [{
          action: 'scanned',
          timestamp: (evidence.scanResults as any).scannedAt,
