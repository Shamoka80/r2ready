  ...
  static async getDashboardKPIs(tenantId: string): Promise<DashboardKPIs> {
    try {
      ...
      const [assessmentCounts] = await db
        .select({
          total: count(),
          inProgress: sql<number>`count(*) FILTER (WHERE ${assessments.status} = 'IN_PROGRESS')`,
          completed: sql<number>`count(*) FILTER (WHERE ${assessments.status} = 'COMPLETED')`,
          needsReview: sql<number>`count(*) FILTER (WHERE ${assessments.status} = 'UNDER_REVIEW')`,
        })
        .from(assessments)
      ...
      const certificationReady = completedAssessments.filter(
        a => (a.overallScore || 0) >= 90
      ).length;
