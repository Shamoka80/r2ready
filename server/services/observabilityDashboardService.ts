...
import { db } from '../db';
import { systemLogs, performanceMetrics, errorLogs, auditLog, assessments, answers } from '@shared/schema';
...
export class ObservabilityDashboardService {
  ...
  private async getSystemHealth(startTime: Date, endTime: Date) {
    ...
    const [activeUsersData] = await db
      ...
      .from(systemLogs)
      .where(and(
        gte(systemLogs.timestamp, new Date(Date.now() - 60 * 60 * 1000)), // Last hour
    ...
    try {
      ...
      const assessmentCounts = await db
        .select({
          ...
          completedAssessments: sql<number>`COUNT(CASE WHEN status = 'COMPLETED' THEN 1 END)`,
          inProgressAssessments: sql<number>`COUNT(CASE WHEN status = 'IN_PROGRESS' THEN 1 END)`,
          newAssessments: sql<number>`COUNT(CASE WHEN created_at >= ${startTime} THEN 1 END)`
        })
        .from(assessments)
      ...
      const errorMetrics = await db
        .select({
          errorCount: sql<number>`COUNT(CASE WHEN level = 'error' THEN 1 END)`,
          warningCount: sql<number>`COUNT(CASE WHEN level = 'warn' THEN 1 END)`,
      ...
      const answerActivity = await db
        .select({
          ...
          uniqueQuestions: sql<number>`COUNT(DISTINCT question_id)`
        })
        .from(answers)
        .where(and(
  ...
  private async getActiveAlerts(): Promise<SystemAlert[]> {
    ...
    const [failedLogins] = await db
      ...
      .where(and(
        ...
        sql`message LIKE '%failed login%' OR message LIKE '%authentication failed%'`
      ));
  ...
  private async getTrends(startTime: Date, endTime: Date): Promise<TrendData[]> {
    ...
    const calculateTrend = (data: { timestamp: Date; value: number }[]): { trend: 'improving' | 'declining' | 'stable'; changePercentage: number } => {
      if (data.length < 2) return { trend: 'stable', changePercentage: 0 };
      ...
      else if (changePercentage < 0) trend = 'improving'; // Lower response time is better
  ...
  async createAlert(alertData: Omit<SystemAlert, 'id' | 'timestamp' | 'resolved'>): Promise<void> {
