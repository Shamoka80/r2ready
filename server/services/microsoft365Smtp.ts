...
class Microsoft365SmtpService {
   ...
   */
  async initialize(): Promise<Transporter | null> {
    try {
      ...
      const hasOAuth2Credentials = !!(
        (process.env.MICROSOFT_365_CLIENT_ID || process.env.MS365_OAUTH_CLIENT_ID) &&
      ...
      this.logger.error('Failed to initialize Microsoft 365 SMTP', error as any);
      this.transporter = null;
      return null;
    }
  }
