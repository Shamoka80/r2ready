...
export class LicenseService {
   ...
   */
  static async getLicenseEntitlements(tenantId: string): Promise<LicenseEntitlements> {
   ...
   */
  static async canAddFacility(tenantId: string): Promise<{ allowed: boolean; reason?: string; currentCount: number; maxAllowed: number }> {
   ...
   */
  private static getFacilityLimitsFromPlan(planId: string, planName: string): { maxFacilities: number; maxSeats: number; canAddFacilities: boolean } {
    ...
    if (businessPlans[planId as keyof typeof businessPlans]) {
      return businessPlans[planId as keyof typeof businessPlans];
    }

    // Check consultant plans
    if (consultantPlans[planId as keyof typeof consultantPlans]) {
      const plan = consultantPlans[planId as keyof typeof consultantPlans];
      return {
