...
class ExportService {
  ...
  static async generatePDF(assessmentId: string, tenantId: string, templateType: string = 'technical-report'): Promise<Buffer> {
    try {
      ...
      // All PDF types use the technical report generator for now
      // The templateProcessor.generatePDFTechnicalReport handles all PDF generation
      return await templateProcessor.generatePDFTechnicalReport(assessmentId, tenantId);
    } catch (error) {
  ...
  static async generateExcel(assessmentId: string, tenantId: string, templateType: string = 'dashboard'): Promise<Buffer> {
    try {
      console.log(`[ExportService] Generating Excel for assessment ${assessmentId}, template: ${templateType}`);
      
      // All Excel types use the dashboard generator
      return await templateProcessor.generateExcelDashboard(assessmentId, tenantId);
    } catch (error) {
      console.error('[ExportService] Excel generation failed:', error);
  ...
  static async generateWord(assessmentId: string, tenantId: string, templateType: string = 'executive-summary'): Promise<Buffer> {
    try {
      console.log(`[ExportService] Generating Word for assessment ${assessmentId}, template: ${templateType}`);
      
      // Use the Word report generator
      return await templateProcessor.generateWordReport(assessmentId, tenantId);
    } catch (error) {
      console.error('[ExportService] Word generation failed:', error);
  ...
  static async generateEmail(assessmentId: string, tenantId: string, templateType: string = 'consultation'): Promise<string> {
    try {
      console.log(`[ExportService] Generating Email for assessment ${assessmentId}, template: ${templateType}`);
      
      // Use the email summary generator
      return await templateProcessor.generateEmailSummary(assessmentId, tenantId);
    } catch (error) {
      console.error('[ExportService] Email generation failed:', error);
