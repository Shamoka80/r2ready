...
export class RateLimitService {
   ...
   */
  private static getRateLimit(resource: string, action: string): { maxRequests: number; windowSeconds: number } {
    const key = `${resource}:${action}` as keyof typeof this.defaultLimits;
    ...
    try {
      ...
      const recentEvents = await db
        ...
        .where(and(
          ...
          eq(rateLimitEvents.resource, resource),
      ...
      const eventData: NewRateLimitEvent = {
        identifier,
        identifierType,
        resource,
        action,
      ...
      if (!allowed && recentEvents.length > 0) {
        const oldestEvent = recentEvents.reduce((oldest, event) => 
          event.createdAt < oldest.createdAt ? event : oldest
        );
   ...
   */
  static async cleanupOldEvents(olderThanHours: number = 24): Promise<{ deletedCount: number }> {
    try {
  ...
  static async getBlockedIdentifiers(
    ...
    try {
      ...
      // Build where clause
      let whereClause: SQL<unknown> | undefined = gte(rateLimitEvents.createdAt, windowStart);
      ...
      const groupedEvents = new Map<string, typeof recentEvents>();
      ...
      for (const [key, events] of groupedEvents) {
        ...
        if (events.length >= limits.maxRequests) {
          ...
          const blockedUntil = new Date(
            oldestEvent.createdAt.getTime() + (limits.windowSeconds * 1000)
          );
