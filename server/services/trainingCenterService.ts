...
interface TrainingModule {
  ...
  title: string;
  description: string;
  category: 'CORE_REQUIREMENT' | 'APPENDIX' | 'ADVANCED' | 'PREPARATION';
...
interface ContentSection {
  ...
  title: string;
  content: string;
  type: 'TEXT' | 'VIDEO' | 'INTERACTIVE' | 'CHECKLIST' | 'WORKSHOP' | 'PRACTICAL' | 'SCENARIO';
  resources?: string[];
}
...
const R2V3_TRAINING_MODULES: TrainingModule[] = [
...
const CERTIFICATION_PREP_PROGRAM: CertificationPrep = {
  id: 'r2v3-cert-prep',
...
class TrainingCenterService {
  ...
  async getTrainingModule(moduleId: string, userId: string): Promise<TrainingModule | null> {
  ...
  async getCertificationPrep(userId: string): Promise<CertificationPrep> {
    return CERTIFICATION_PREP_PROGRAM;
  }

  // Get knowledge base articles
  async getKnowledgeBase(category?: string, searchTerm?: string): Promise<any[]> {
    const knowledgeBase = [
      {
        id: 'kb-r2v3-overview',
  ...
  async completeModuleAssessment(userId: string, moduleId: string, answers: any[]): Promise<any> {
    const module = R2V3_TRAINING_MODULES.find(m => m.id === moduleId);
  ...
  async startModule(userId: string, moduleId: string): Promise<any> {
    const module = R2V3_TRAINING_MODULES.find(m => m.id === moduleId);
    if (!module) {
  ...
  async updateProgress(userId: string, moduleId: string, progressData: any): Promise<any> {
  ...
  async updateTutorialProgress(userId: string, tutorialId: string, stepId: string, responses: any): Promise<any> {
  ...
  async getComprehensiveProgress(userId: string): Promise<any> {
    return {
      userId,
      overallProgress: 42,
      completedModules: 4,
