...
export class ExecutiveSummaryService {
   ...
   */
  static async generateExecutiveSummary(assessmentId: string): Promise<{
    summary: string;
    keyFindings: string[];
   ...
   */
  private static async getAssessmentForSummary(assessmentId: string) {
   ...
   */
  private static calculateSummaryMetrics(answers: any[]): any {
    const total = answers.length;
    const compliant = answers.filter((a: any) => a.compliance === 'COMPLIANT').length;
    const nonCompliant = answers.filter((a: any) => a.compliance === 'NON_COMPLIANT').length;
    const criticalGaps = answers.filter((a: any) => a.questionCategory === 'critical' && a.compliance === 'NON_COMPLIANT').length;

    // Section analysis
    const sections = [...new Set(answers.map((a: any) => a.questionCategoryName || a.questionCategory).filter(Boolean))];
    const sectionPerformance = sections.map(section => {
      const sectionAnswers = answers.filter((a: any) => (a.questionCategoryName || a.questionCategory) === section);
      const sectionCompliant = sectionAnswers.filter((a: any) => a.compliance === 'COMPLIANT').length;
      return {
        section,
   ...
   */
  private static generateNarrativeSummary(assessment: any, metrics: any): string {
...
Based on industry benchmarks and certification body requirements, we estimate a ${this.estimateSuccessProbability(complianceRate)}% probability of successful certification following implementation of our recommended corrective action plan.
   ...
   */
  private static extractKeyFindings(answers: any[], metrics: any): string[] {
    const findings = [];
    ...
    // Section performance
    const poorSections = metrics.sectionPerformance.filter((s: any) => s.compliance < 70);
    if (poorSections.length > 0) {
      findings.push(`${poorSections.length} compliance domains require focused improvement initiatives`);
    }

    // Positive findings
    const strongSections = metrics.sectionPerformance.filter((s: any) => s.compliance >= 90);
   ...
   */
  private static assessBusinessImpact(metrics: any): string {
   ...
   */
  private static generateStrategicRecommendations(answers: any[], metrics: any): string[] {
    ...
    // Section-specific
    const weakSections = metrics.sectionPerformance.filter((s: any) => s.compliance < 60);
   ...
   */
  private static defineNextSteps(metrics: any): string[] {
    const steps = [];
   ...
   */
  private static generateInvestmentAnalysis(answers: any[], metrics: any): any {
   ...
   * Helper methods
   */
  private static determineReadinessLevel(compliant: number, total: number, highRisk: number, criticalGaps: number): string {
  ...
  private static estimateSuccessProbability(compliance: number): number {
    if (compliance >= 95) return 95;
    if (compliance >= 90) return 85;
    if (compliance >= 80) return 70;
    if (compliance >= 70) return 55;
    return 35;
  }

  private static estimateProjectTimeline(metrics: any): string {
    const gaps = metrics.gaps;
    const criticalGaps = metrics.criticalGaps;
    const complexity = criticalGaps + (gaps * 0.5);

    if (complexity <= 10) return '8-12 weeks to certification readiness';
    if (complexity <= 25) return '12-16 weeks to certification readiness';
    if (complexity <= 40) return '16-24 weeks to certification readiness';
    return '24+ weeks to certification readiness - phased approach recommended';
  }

  private static estimatePaybackPeriod(investment: number): string {
    if (investment <= 25000) return '6-12 months through operational efficiency gains';
    if (investment <= 50000) return '12-18 months through market access and efficiency';
    return '18-24 months through expanded market opportunities';
  }

  private static calculateRiskMitigationValue(metrics: any): string {
    const compliance = metrics.overallCompliance;
    if (compliance >= 90) return '$50,000 - $100,000 in avoided compliance risks';
    if (compliance >= 75) return '$100,000 - $250,000 in avoided compliance risks';
...
export async function generateExecutiveSummary(assessmentId: string): Promise<ExecutiveSummary> {
  ...
  const answeredQuestions = assessmentAnswers.filter((a: any) => a.value).length;
  ...
  let complianceStatus: 'COMPLIANT' | 'NEEDS_IMPROVEMENT' | 'NON_COMPLIANT';
  ...
  const lowScoringAnswers = assessmentAnswers.filter((a: any) => a.score !== null && a.score < 3);
  ...
  const criticalIssues = assessmentAnswers.filter((a: any) => 
    a.compliance === 'NON_COMPLIANT' && (a.score === null || a.score < 3)
  );
