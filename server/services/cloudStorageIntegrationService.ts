...
export class CloudStorageIntegrationService {
  ...
  async uploadToGoogleDrive(fileBuffer: Buffer, fileName: string, mimeType: string): Promise<UploadResult> {
  ...
  async uploadToDropbox(fileBuffer: Buffer, fileName: string): Promise<UploadResult> {
    ...
    try {
      ...
      throw new Error(`Dropbox upload failed: ${(error as Error).message}`);
    }
  }

  async uploadToAzureBlob(fileBuffer: Buffer, fileName: string, mimeType: string): Promise<UploadResult> {
    ...
    try {
      ...
      throw new Error(`Azure Blob upload failed: ${(error as Error).message}`);
    }
  }

  async downloadFile(provider: string, fileId: string, encryptionKey?: string): Promise<Buffer> {
    let fileBuffer: Buffer;

    switch (provider) {
      case 'google_drive':
        if (!this.googleStorage) throw new Error('Google Cloud Storage not configured');
      ...
      case 'dropbox':
        ...
        fileBuffer = (downloadResponse.result as any).fileBinary as Buffer;
        break;

      case 'azure_blob':
        if (!this.azureBlobClient) throw new Error('Azure Blob Storage not configured');
        const containerName = process.env.AZURE_CONTAINER_NAME || 'rur2-evidence';
        const containerClient = this.azureBlobClient.getContainerClient(containerName);
  ...
  async checkQuota(provider: string): Promise<{ used: number; limit: number; available: number }> {
    switch (provider) {
      case 'google_drive':
      ...
      case 'dropbox':
        ...
        const allocation = (usage.result.allocation as any).allocated || 1000000000000;
        return {
  ...
  private async encryptFile(data: Buffer): Promise<{ encryptedData: Buffer; encryptionKey: string }> {
  ...
  private decryptFile(encryptedData: Buffer, encryptionKeyBase64: string): Buffer {
    const algorithm = 'aes-256-gcm';
  ...
  async getProviderStatus(): Promise<CloudProvider[]> {
