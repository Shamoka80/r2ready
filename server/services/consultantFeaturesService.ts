...
export interface ConsultantDashboard {
  ...
  performanceMetrics: ConsultantMetrics;
  clientAlerts: ClientAlert[];
}

export interface DashboardActivity {
  id: string;
  type: 'assessment_completed' | 'client_added' | 'audit_scheduled' | 'certification_achieved';
  clientName: string;
  facilityName?: string;
  description: string;
  timestamp: Date;
...
export interface ClientAlert {
  ...
  clientId: string;
  facilityId?: string;
  type: 'compliance_issue' | 'deadline_approaching' | 'assessment_overdue' | 'training_required';
...
export class ConsultantFeaturesService {
   ...
   */
  static async getConsultantDashboard(userId: string): Promise<ConsultantDashboard> {
    try {
      ...
      if (clientOrgIds.length > 0) {
        ...
        const assessmentCounts = await db
          ...
          .from(assessments)
          .where(and(
            eq(assessments.tenantId, consultantTenantId),
            inArray(assessments.clientOrganizationId, clientOrgIds)
          ))
          .groupBy(assessments.status);

        activeAssessments = assessmentCounts
          .filter((a: any) => a.status === 'IN_PROGRESS' || a.status === 'DRAFT')
          .reduce((sum: number, a: any) => sum + Number(a.count), 0);
      ...
      const performanceMetrics: ConsultantMetrics = {
      ...
      const dashboard: ConsultantDashboard = {
        clientSummary,
        recentActivity,
        performanceMetrics,
        clientAlerts
   ...
   * CRITICAL: Filters by consultantTenantId to ensure data isolation
   */
  static async getClientPortfolio(userId: string, specificClientOrgId?: string): Promise<ClientPortfolio[]> {
    try {
      // Get user's tenant
      const user = await db.query.users.findFirst({
        where: eq(users.id, userId),
      ...
      if (specificClientOrgId) {
        clientOrgsWhere = and(
          eq(clientOrganizations.consultantTenantId, consultantTenantId),
          eq(clientOrganizations.id, specificClientOrgId)
        )!;
      }
      ...
      for (const clientOrg of clientOrgs) {
        ...
        const clientAssessments = await db.query.assessments.findMany({
          where: and(
            eq(assessments.tenantId, consultantTenantId),
            eq(assessments.clientOrganizationId, clientOrg.id)
          ),
   ...
   */
  static async generateWhiteLabelReport(
    ...
    try {
      ...
      const assessment = await db.query.assessments.findFirst({
        where: eq(assessments.id, assessmentId) as any
      });
      ...
      const hasAccess = consultantClients ? await db
        .select()
        .from(consultantClients as any)
        .where(and(
          eq(consultantClients.consultantUserId, consultantUserId),
          eq(consultantClients.clientTenantId, assessment.tenantId)
        ) as any)
        .limit(1)
  ...
  static async getAdvancedAnalytics(
    ...
    assessmentTrends: { month: string; completed: number; inProgress: number }[];
    complianceDistribution: { score: string; count: number }[];
    revenueProjection: { month: string; projected: number; actual: number }[];
    industryBenchmarks: { metric: string; yourValue: number; industry: number }[];
  }> {
    try {
      // Get client relationships within time range
      const clientGrowthData: any = await db
        .select({
        ...
        .where(
          and(
            eq(consultantClients.consultantUserId, consultantUserId),
            sql`start_date >= ${timeRange.startDate}`,
            sql`start_date <= ${timeRange.endDate}`
          ) as any
        )
        .groupBy(sql`to_char(start_date, 'YYYY-MM')` as any)
        .orderBy(sql`to_char(start_date, 'YYYY-MM')` as any);

      const clientGrowth = clientGrowthData.map((row: any) => ({
        month: row.month,
        clients: Number(row.clients)
      }));

      // Get assessment trends
      const clientIds: any = await db
        .select({ clientId: consultantClients.clientTenantId })
        .from(consultantClients as any)
        .where(eq(consultantClients.consultantUserId, consultantUserId) as any);

      const clientTenantIds = clientIds.map((c: any) => c.clientId);

      let assessmentTrends: { month: string; completed: number; inProgress: number }[] = [];
      
      if (clientTenantIds.length > 0) {
        const assessmentData: any = await db
          .select({
          ...
          .where(
            and(
              inArray(assessments.tenantId, clientTenantIds),
              sql`created_at >= ${timeRange.startDate}`,
              sql`created_at <= ${timeRange.endDate}`
            ) as any
          )
          .groupBy(sql`to_char(created_at, 'YYYY-MM')` as any, assessments.status as any)
          .orderBy(sql`to_char(created_at, 'YYYY-MM')` as any);

        // Process assessment trends
        const trendMap = new Map<string, { completed: number; inProgress: number }>();
   ...
   */
  static async getAdvancedConsultantAnalytics(
    consultantUserId: string,
    timeRange: { startDate: Date; endDate: Date }
  ): Promise<{
    ...
    try {
      ...
      const clientRelationships: any = await db
        .select()
        .from(consultantClients as any)
        .where(eq(consultantClients.consultantUserId, consultantUserId) as any);

      const clientIds = clientRelationships.map((c: any) => c.clientTenantId);
   ...
   */
  static async getWhiteLabelOptions(consultantUserId: string): Promise<{
   ...
   */
  static async updateWhiteLabelBranding(
    consultantUserId: string,
  ...
  static async addClient(
    ...
    relationshipType: 'primary' | 'secondary' | 'partner' = 'primary'
  ): Promise<void> {
    try {
      ...
      const existingRelationship = await db
        ...
        .where(and(
          eq(consultantClients.consultantUserId, consultantUserId),
          eq(consultantClients.clientTenantId, clientTenantId)
        ))
        .limit(1)
        .then(rows => rows[0]);
      ...
      const newRelationship: NewConsultantClient = {
        consultantUserId,
        clientTenantId,
        relationshipType,
        startDate: new Date(),
