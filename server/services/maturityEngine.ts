...
export interface MaturityScore {
  ...
  maturityLevel: 'BASIC' | 'DEVELOPING' | 'ESTABLISHED' | 'ADVANCED' | 'OPTIMIZED';
  dimensionDetails: MaturityDimension[];
}

export interface MaturityDimension {
  name: string;
  category: 'BCP' | 'CI' | 'STAKEHOLDER';
  score: number;
  level: string;
  strengths: string[];
...
export interface MaturityQuestionMetrics {
  questionId: string;
  questionText: string;
  category: string;
  maturityCategory: 'BCP' | 'CI' | 'STAKEHOLDER';
  score: number;
  weight: number;
...
export class MaturityEngine {
  ...
  private static readonly MATURITY_THRESHOLDS = {
    'BASIC': 0,
    'DEVELOPING': 40,
    'ESTABLISHED': 60,
    'ADVANCED': 80,
   ...
   */
  static async calculateMaturityScore(assessmentId: string): Promise<MaturityScore> {
    ...
    const overallMaturity = (
      bcpScore.score * this.DIMENSION_WEIGHTS.BCP +
  ...
  private static async getMaturityQuestions(assessmentId: string): Promise<MaturityQuestionMetrics[]> {
    ...
    return maturityAnswers.map(answer => {
      ...
      return {
        ...
        maturityCategory: (questionObj?.maturityCategory as 'BCP' | 'CI' | 'STAKEHOLDER') || 'CI',
        score,
        weight,
   ...
   */
  private static async calculateDimensionScore(
    assessmentId: string,
    dimension: 'BCP' | 'CI' | 'STAKEHOLDER',
    allMaturityQuestions: MaturityQuestionMetrics[]
  ): Promise<MaturityDimension> {
    const dimensionQuestions = allMaturityQuestions.filter(
      q => q.maturityCategory === dimension
    );
    ...
    const weightedScore = dimensionQuestions.reduce(
      (sum, q) => sum + (q.score * q.weight), 
      0
    );
   ...
   */
  private static determineMaturityLevel(score: number): 'BASIC' | 'DEVELOPING' | 'ESTABLISHED' | 'ADVANCED' | 'OPTIMIZED' {
    if (score >= this.MATURITY_THRESHOLDS.OPTIMIZED) return 'OPTIMIZED';
    if (score >= this.MATURITY_THRESHOLDS.ADVANCED) return 'ADVANCED';
    if (score >= this.MATURITY_THRESHOLDS.ESTABLISHED) return 'ESTABLISHED';
    if (score >= this.MATURITY_THRESHOLDS.DEVELOPING) return 'DEVELOPING';
    return 'BASIC';
  }
   ...
   */
  private static getDimensionName(dimension: 'BCP' | 'CI' | 'STAKEHOLDER'): string {
    const names = {
   ...
   */
  private static createEmptyMaturityScore(): MaturityScore {
    return {
      overallMaturity: 0,
   ...
   */
  static async saveMaturityScore(
    assessmentId: string, 
    ...
    const result = await db.insert(maturityScores)
      .values({
        ...
        dimensionDetails: maturityScore.dimensionDetails as any
      })
      .returning();
   ...
   */
  static async getMaturityScore(assessmentId: string): Promise<MaturityScore | null> {
    ...
    return {
      ...
      maturityLevel: score.maturityLevel as any,
      dimensionDetails: score.dimensionDetails as any
    };
  }
   ...
   */
  static async getMaturityTrend(assessmentId: string): Promise<{
    dates: string[];
