...
export async function autoMigrateSchema(): Promise<MigrationResult> {
...
async function generateMigrationSQL(error: string): Promise<Migration | null> {
...
async function generateAddColumnMigration(tableName: string, columnName: string): Promise<Migration> {
  // Define type mappings based on common patterns
  const columnTypeMappings: Record<string, string> = {
...
export function getColumnDefinition(tableName: string, columnName: string) {
