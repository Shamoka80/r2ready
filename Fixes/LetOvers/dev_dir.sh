# Part 1 — deps & dirs
set -euo pipefail
mkdir -p ./Fixes/reports
python3 - <<'PY'
import os, sys
for p in ("./Fixes/questions.csv","./Fixes/pdf_temp_export.pdf"):
    if not os.path.exists(p):
        sys.exit(f"Missing required file: {p}")
print("OK: source files exist")
PY
pip install --quiet 'PyPDF2>=3.0.0'
echo "OK: deps installed"



# Part 2 — write the generator (Phase 0)
cat > phase0_generate_reports.py <<'PY'
#!/usr/bin/env python3
...
if __name__=="__main__":
    sys.exit(main())
PY
chmod +x phase0_generate_reports.py
echo "OK: generator written"


# Part 3 — run & quick checks
set -euo pipefail
python3 phase0_generate_reports.py

echo "==> coverage head"
head -n 20 ./Fixes/reports/coverage_report.csv || true

echo "==> binding map field count"
python3 - <<'PY'
import json; d=json.load(open('./Fixes/reports/binding_map_v1.json'))
