# 1) Set env (paste your NEW token)
export GITHUB_TOKEN='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'
export OWNER='Shamoka80'
export REPO='R2v3APP'

# 2) Create + run a strict CI watcher/diagnostic
cat > watch_ci_secure.sh <<'BASH'
#!/usr/bin/env bash
set -euo pipefail
...
PY
BASH
chmod +x watch_ci_secure.sh && ./watch_ci_secure.sh